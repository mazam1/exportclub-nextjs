      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
          {categories.map((c) => (
            <Link
              key={c.name}
              href={(c as any).href || "/products?category=men"}
            ...
            >
              <CategoryImage
                src={c.img}
                fallbackSrc={(c as any).fallbackImg}
                alt={c.alt || c.name}
                fill
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="rounded-full border border-[#d9d9d9] bg-white/85 px-3 py-1 text-[11px] font-medium text-black shadow-sm">
                  {c.name}
                </span>
              </div>
            </Link>
